...
export function FilterBar({ view }: { view: string }) {
  ...
  const setParam = useCallback(
    (key: string, value: string) => {
      ...
      next.delete("cursor");
      startTransition(() => router.replace(`/${view}?${next.toString()}`, { scroll: false }));
    },
    [params, router, view],
  );
  ...
  return (
    ...
    >
      <label className="flex min-w-52 flex-1 items-center gap-2 rounded-sm border border-line bg-base-raised px-2 py-1.5">
      ...
      <label className="flex items-center gap-1 text-xs text-ink-muted">
        ...
        <select
          ...
          className="rounded-sm border border-line bg-base-raised px-2 py-1.5 text-xs"
